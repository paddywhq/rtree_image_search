...
//K近邻查找
public:
  void KSearch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], int k, DATATYPE* a_data);
  int iocount;  //I/O计数器

protected:
  double mindistance; //查找结果中距离查找区域的最大欧拉距离
  Branch* ksearchresult; //储存查找结果
  int ksearchcount; //查找结果数量

  double EulersDistanceMin(Rect a_rectA, Rect a_rectB); //计算两个矩形区域最短欧拉距离
  double EulersDistanceMax(Rect a_rectA, Rect a_rectB); //计算两个矩形区域最长欧拉距离
  void KSearch(Node* a_node, Rect a_rect, int k);
};
...
//K邻近查找
RTREE_TEMPLATE
void RTREE_QUAL::KSearch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], int k, DATATYPE* a_data)
{
  mindistance = 0;
  ksearchresult = new(Branch[k]);
  ksearchcount = 0;
  iocount = 1;
  ...
  for(int i=0; i<k; ++i)
  {
	for(int j=i+1; j<k; ++j)
	{
	  if(EulersDistanceMin(ksearchresult[i].m_rect, rect) > EulersDistanceMin(ksearchresult[j].m_rect, rect))
	  {
		temp = ksearchresult[i];
		ksearchresult[i] = ksearchresult[j];
		ksearchresult[j] = temp;
	  }
	}
  }
  for(int i=0; i<k; ++i)
  {
	a_data[i] = ksearchresult[i].m_data;
  }

  delete []ksearchresult;
}

//计算两个矩形区域最短欧拉距离
RTREE_TEMPLATE
double RTREE_QUAL::EulersDistanceMin(Rect a_rectA, Rect a_rectB)
{
  double dis = 0;
  for(int axis=0; axis<NUMDIMS; ++axis)
  {
	if(a_rectA.m_max[axis] < a_rectB.m_min[axis])
	{
	  dis += (a_rectA.m_max[axis] - a_rectB.m_min[axis]) * (a_rectA.m_max[axis] - a_rectB.m_min[axis]);
	}
	else if(a_rectA.m_min[axis] > a_rectB.m_max[axis])
	{
	  dis += (a_rectA.m_min[axis] - a_rectB.m_max[axis]) * (a_rectA.m_min[axis] - a_rectB.m_max[axis]);
	}
  }
  return sqrt(dis);
}
...
//计算两个矩形区域最长欧拉距离
RTREE_TEMPLATE
double RTREE_QUAL::EulersDistanceMax(Rect a_rectA, Rect a_rectB)
{
  double dis = 0;
  for(int axis=0; axis<NUMDIMS; ++axis)
  {
	if(abs(a_rectA.m_max[axis] - a_rectB.m_min[axis]) > abs(a_rectA.m_min[axis] - a_rectB.m_max[axis]))
	{
	  dis += (a_rectA.m_max[axis] - a_rectB.m_min[axis]) * (a_rectA.m_max[axis] - a_rectB.m_min[axis]);
	}
	else
...
RTREE_TEMPLATE
void RTREE_QUAL::KSearch(Node* a_node, Rect a_rect, int k)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ...
  if(a_node->IsInternalNode()) // This is an internal node in the tree
  {
    ...
    {
	  iocount++;
	  if(EulersDistanceMin(a_node->m_branch[index].m_rect, a_rect) < mindistance || ksearchcount < k)
	  {
	    KSearch(a_node->m_branch[index].m_child, a_rect, k);
	  }
	}
  }
  else // This is a leaf node
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
	  ...
	  if(ksearchcount < k)
	  {
		ksearchresult[ksearchcount] = a_node->m_branch[index];
		ksearchcount++;
		if(EulersDistanceMin(a_node->m_branch[index].m_rect, a_rect) > mindistance)
		{
		  mindistance = EulersDistanceMin(a_node->m_branch[index].m_rect, a_rect);
		}
	  }
      else if(EulersDistanceMin(a_node->m_branch[index].m_rect, a_rect) < mindistance)
      {
		for(int i=0; i<k; ++i)
		{
		  if(EulersDistanceMin(ksearchresult[i].m_rect, a_rect) == mindistance)
		  {
			ksearchresult[i] = a_node->m_branch[index];
			break;
		  }
		}
		mindistance = 0;
		for(int i=0; i<k; ++i)
		{
		  if(EulersDistanceMin(ksearchresult[i].m_rect, a_rect) > mindistance)
		  {
			mindistance = EulersDistanceMin(ksearchresult[i].m_rect, a_rect);
		  }
		}
      }
    }
